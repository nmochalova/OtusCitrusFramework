...
**TestHelperHTTP** - пример http-хелпера, который умеет обращаться отправлять запросы от имени своего http-клиента на некий endpoint, конвертировать полученный ответ из Json в pojo и сравнивать его с ожидаемым ответом.

**TestHelperSOAP** - пример soap-хелпера, который умеет обращаться отправлять запросы от имени своего soap-клиента на некий endpoint, конвертировать полученный ответ из xml в pojo и сравнивать его с ожидаемым ответом.

**TestHelperWSS** - пример WebSocket-хелпера, который умеет обращаться отправлять запросы от имени своего wss-клиента на некий открытый WebSocket сервис от Binance, конвертировать полученный ответ из Json в pojo и сравнивать его с ожидаемым ответом.
